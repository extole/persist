...
public final class AnnotationTableMapping extends Mapping {
    ...
    private final String[] writableColumns; // columns that can be used in INSERT/UPDATE (not autoGenerated and not readOnly)
    private final String[] writableNotPrimaryKeys; // non-primary-key columns that can be used in UPDATE (not readOnly)
    ...
    public AnnotationTableMapping(final Class<?> objectClass) {
        ...
        final List<String> writableColumnsList = new ArrayList<>();
        final List<String> writableNotPrimaryKeysList = new ArrayList<>();
        ...
        for (String columnName : columns) {
            ...
            // writable columns are those that are neither autoGenerated nor readOnly
            if (!autoGeneratedColumnsTemp.contains(columnName) && !readOnlyColumnsTemp.contains(columnName)) {
                writableColumnsList.add(columnName);
            }
        }
        
        // writable non-primary keys (for UPDATE operations)
        for (String columnName : notPrimaryKeys) {
            if (!readOnlyColumnsTemp.contains(columnName)) {
                writableNotPrimaryKeysList.add(columnName);
            }
        }
        ...
        writableColumns = toArray(writableColumnsList);
        writableNotPrimaryKeys = toArray(writableNotPrimaryKeysList);

        // assemble sql blocks to be used by crud sql statements

        final String allColumns = join(columns, "", ",");
        final String writableColumnsStr = join(writableColumns, "", ",");
        final String writablePlaceholders = multiply("?", writableColumns.length, ",");
        final String where = join(primaryKeys, "=?", " and ");
        final String updateSet = join(writableNotPrimaryKeys, "=?", ",");
        ...
        // INSERT uses only writable columns (excludes both autoGenerated and readOnly)
        insertSql = "insert into " + tableName + "(" + writableColumnsStr + ") values (" + writablePlaceholders + ")";

        // UPDATE uses only writable non-primary keys (excludes readOnly)
    ...
    public String[] getWritableNotPrimaryKeys() {
        return writableNotPrimaryKeys;
    }
