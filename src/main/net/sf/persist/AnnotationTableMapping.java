...
public final class AnnotationTableMapping extends Mapping {
    ...
    private final String[] selectColumns; // columns for SELECT operations (all columns)
    private final String[] insertColumns; // columns for INSERT operations (not autoGenerated and not readOnly)
    private final String[] updateColumns; // columns for UPDATE operations (non-primary-key columns that are not readOnly)
    ...
    public AnnotationTableMapping(final Class<?> objectClass) {
        ...
        final Set<String> readOnlyColumnsTemp = new HashSet<>();
        ...
        for (String fieldName : fields) {
            ...
            if (annotation != null && annotation.readOnly()) {
                readOnlyColumnsTemp.add(columnName);
            }
        }
        ...
        final List<String> readOnlyColumnsList = new ArrayList<>();
        final List<String> selectColumnsList = new ArrayList<>();
        final List<String> insertColumnsList = new ArrayList<>();
        final List<String> updateColumnsList = new ArrayList<>();
        ...
        for (String columnName : columns) {
            ...
            if (readOnlyColumnsTemp.contains(columnName)) {
                readOnlyColumnsList.add(columnName);
            }
            
            // All columns are for SELECT
            selectColumnsList.add(columnName);
            
            // INSERT columns are those that are neither autoGenerated nor readOnly
            if (!autoGeneratedColumnsTemp.contains(columnName) && !readOnlyColumnsTemp.contains(columnName)) {
                insertColumnsList.add(columnName);
            }
        }
        
        // UPDATE columns: non-primary keys that are not readOnly
        for (String columnName : notPrimaryKeys) {
            if (!readOnlyColumnsTemp.contains(columnName)) {
                updateColumnsList.add(columnName);
            }
        }
        
        notAutoGeneratedColumns = toArray(notAutoGeneratedColumnsList);
        autoGeneratedColumns = toArray(autoGeneratedColumnsList);
        readOnlyColumns = toArray(readOnlyColumnsList);
        selectColumns = toArray(selectColumnsList);
        insertColumns = toArray(insertColumnsList);
        updateColumns = toArray(updateColumnsList);

        // assemble sql blocks to be used by crud sql statements

        final String selectColumnsStr = join(selectColumns, "", ",");
        final String insertColumnsStr = join(insertColumns, "", ",");
        final String insertPlaceholders = multiply("?", insertColumns.length, ",");
        final String where = join(primaryKeys, "=?", " and ");
        final String updateSet = join(updateColumns, "=?", ",");

        // assemble crud sql statements

        selectSql = "select " + selectColumnsStr + " from " + tableName + " where " + where;
        selectAllSql = "select " + selectColumnsStr + " from " + tableName;

        // INSERT uses only insert columns (excludes both autoGenerated and readOnly)
        insertSql = "insert into " + tableName + "(" + insertColumnsStr + ") values (" + insertPlaceholders + ")";

        // UPDATE uses only update columns (non-primary keys that are not readOnly)
